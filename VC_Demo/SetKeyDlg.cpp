...
int CSetKeyDlg::CheckKey(PSTR szKey)
{
	...
	CButton *pRad = (CButton*)GetDlgItem(IDC_RAD_ASCII);
	if (BST_CHECKED != pRad->GetCheck())	//hex mode //16进制模式
	{
		bASCII = FALSE;
	}
	...
	if (0 == iCurSel)	//check key length, just surpport 128 bits now //检查密钥长度，目前只支持128位模式
	{
	...
	if (!bASCII)
	{
		...
		if (i != iLength)	//input invalid //输入非法
		{
			return -1;
		}
...
void CSetKeyDlg::HexToASCII(PSTR szKey)
{
	...
	{
		if (szKey[i] >= 65)		//translate charater to hex value //将字母转化为对应的16进制数值
		{
			szKey[i] -= 55;
		}
		else					//trslate number to hex value //将数字转化为对应的16进制数值
		{
			szKey[i] -= 48;
		}
		...
		szKey[i / 2] = szKey[i] << 4 | szKey[i + 1]; //two charater in Hex is one charater in ASCII //Hex下两个字符表示ASCII下一个字符
	}
}
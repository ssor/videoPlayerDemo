...
void CSeek::OnSeek() 
{
	...
	if(m_nSeekType == 0)
	{
		...
		NAME(VN_PLAY_SetCurrentFrameNum)(m_lPort, m_nValue);
	}
	else if(m_nSeekType == 1)
	{
		...
		NAME(VN_PLAY_SetPlayedTimeEx)(m_lPort, m_nValue*1000);
	}
...
BOOL CSeek::OnInitDialog() 
{
	...
	m_dwMaxTime     = NAME(VN_PLAY_GetFileTime)(m_lPort);
	m_dwMaxFrameNum = NAME(VN_PLAY_GetFileTotalFrames)(m_lPort);
	CString csRange;

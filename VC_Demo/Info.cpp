...
void CInfo::OnRefresh() 
{
	...
	int nRows = m_dwRows;
		...
		//invalid after play()
	BOOL bOverlay=NAME(VN_PLAY_GetOverlayMode)(m_lPort);
	if(!m_bInit)
	{
	...
	if(bOverlay)
	{
		m_strTemp.Format("yes   The color key:0x%X",NAME(VN_PLAY_GetColorKey)(m_lPort));
	}
	else
	{
		m_strTemp.Format("No");
	...
	m_strTemp.Format("%d",NAME(VN_PLAY_GetDisplayBuf)(m_lPort));
	m_ctrlListInfo.SetItemText(nRows,1,m_strTemp);
	nRows ++;
	...
	m_strTemp.Format("%x",NAME(VN_PLAY_GetVolume)(m_lPort));
	m_ctrlListInfo.SetItemText(nRows,1,m_strTemp);
	nRows ++;
	...
	NAME(VN_PLAY_GetPictureQuality)(m_lPort,&bHiQuality);
	if(bHiQuality)
	{
		m_strTemp="High";
	}
	...
	DWORD nTimer;
	NAME(VN_PLAY_GetTimerType)(m_lPort,&nTimer,NULL);
	if(nTimer==TIMER_1)
	{
		m_strTemp="TIMER_1";
	}
	...
	DWORD nVideoRender =NAME(VN_PLAY_GetBufferValue)(m_lPort,BUF_VIDEO_RENDER);
	DWORD nAudioRender =NAME(VN_PLAY_GetBufferValue)(m_lPort,BUF_AUDIO_RENDER);
	DWORD nVideoSource =NAME(VN_PLAY_GetBufferValue)(m_lPort,BUF_VIDEO_SRC);
	DWORD nAudioSource =NAME(VN_PLAY_GetBufferValue)(m_lPort,BUF_AUDIO_SRC);
...
BOOL CInfo::OnInitDialog() 
{
	...
	if(nVal>=1)
	{
		...
		for(DWORD nCount=1;nCount<=nVal;nCount++) 
		{
			...
			HMONITOR hMonitor;
			NAME(VN_PLAY_GetDDrawDeviceInfo)(nCount,pDriverDesp,nNameLen,pDriverName,nNameLen,&hMonitor);

	...
	: CDialog(CPlayerDlg::IDD, pParent)
{
	...
	m_hIcon	                  =	AfxGetApp()->LoadIcon(IDI_ICON_VISIONDIGI);
...
//	HMONITOR hMonitor;
	...
	NAME(VN_PLAY_InitDDrawDevice)();
	DWORD nVal=NAME(VN_PLAY_GetDDrawDeviceTotalNums)();
	if(nVal > 1)
	{
...
//			NAME(VN_PLAY_GetDDrawDeviceInfo)(nCount, chDriverDesp, 50, chDriverName, 50, &hMonitor);
	...
	{
		ZeroMemory(chDriverDesp, 50);
//		NAME(VN_PLAY_GetDDrawDeviceInfo)(0, chDriverDesp, 50, chDriverName, 50, &hMonitor);
...
// 	
	...
	NAME(VN_PLAY_SetDisplayCallBack)(m_lPort, DisplayCBFun,(long)this);
	// set the wave audio call back funtion;
//	NAME(VN_PLAY_SetAudioCallBack)(m_lPort, WaveCBFun, (long)this);



	NAME(VN_PLAY_SetDDrawDevice)(m_lPort, 0);
	...
	UpdateData(FALSE);
	SortControl();
	NAME(VN_PLAY_SetVolume)(m_lPort,WORD(m_SoundSlider.GetPos()));
	m_bInited = TRUE;

	DWORD  ThreadID;
	m_hUDPThread = CreateThread(NULL,0,setupUDPServer,0,0,&ThreadID);
    //setupUDPServer();
...
#ifdef _FILE_POS
		NAME(VN_PLAY_SetPlayPos)(m_lPort, (float)nPlayPos/(float)PLAYER_SLIDER_MAX);
#else
 		nTime = nPlayPos * 1000 / PLAYER_SLIDER_MAX * m_dwMaxFileTime;
 		NAME(VN_PLAY_SetPlayedTimeEx)(m_lPort,nTime);
#endif
//		DrawStatus();
		break;
...
#ifdef _WAVE_ADJ
		NAME(VN_PLAY_AdjustWaveAudio)(m_lPort, m_SoundSlider.GetPos());
#else
		NAME(VN_PLAY_SetVolume)(m_lPort, WORD(m_SoundSlider.GetPos()));
#endif
		break;
...
//				LONG nPicWidth  = 352;
//				LONG nPicHeight = 288;
//				NAME(VN_PLAY_GetPictureSize)(m_lPort, &nPicWidth, &nPicHeight);
...
{
	// TODO: Add your message handler code here and/or call default
	NAME(VN_PLAY_Stop)(m_lPort);
	NAME(VN_PLAY_CloseFile)(m_lPort);
	NAME(VN_PLAY_RealeseDDraw)();

#if (WINVER > 0x0400)
	NAME(VN_PLAY_ReleaseDDrawDevice)();
#endif
...
//	UNREFERENCED_PARAMETER(pBuf);
	CPlayerDlg* dlg = (CPlayerDlg*)nReceaved;
    ...
    if(nSize <= dlg->m_picData.nCapacity)
	{
		...
		if((dlg->m_picData.isHasData == 1) && (savePic() == 1))
		{
			dlg->GetPic(dlg->m_picData.buf,
				        dlg->m_picData.nSize,
						dlg->m_picData.nWidth,
						dlg->m_picData.nHeight,
						dlg->m_picData.nType);
		}
		/*
		if(savePic() == 1)
...
/*	if(dlg->m_bCapImage)
	{
		dlg->GetPic(pBuf,nSize,nWidth,nHeight,nType);
		dlg->m_bCapImage = FALSE;
	}*/
}
...
{
//	DWORD dwTime = VN_PLAY_GetSpecialData(nPort);
...
void CALLBACK SourceBufFun(long nPort,DWORD nBufSize,DWORD dwUser,void*pContext)
{
	...
	NAME(VN_PLAY_ResetSourceBufFlag)(nPort);
...
//	pOwner->m_pMainMenu->EnableMenuItem(IDM_CUT_FILE,MF_ENABLED);
	...
	VN_PLAY_GetKeyFramePos(m_lPort, 100, BY_FRAMENUM, &stFramePos);
}
...
DWORD WINAPI InputStreamThread( LPVOID lpParameter)
{
	...
	{	
		if(!bBufFull)
		{
			...
			while ( !NAME(VN_PLAY_InputData)(m_lPort, pBuf, dwDataLen) )
			{
				if ( VN_PLAY_GetLastError(m_lPort) == PLAY_BUF_OVER )
				{
					Sleep(10);
					continue;
				}
...
//			if(!NAME(VN_PLAY_InputData)(m_lPort, pBuf, dwDataLen)) //如果数据长度输入dwSize,播放器马上崩溃
//			{
//				bBufFull = TRUE;
//				ResetEvent(pOwner->m_hEventInput);
//			}
...
void CPlayerDlg::OpenDavFile(char *fileName)
{
	Close();
	SetState();
	...
	try
	{
		NAME(VN_PLAY_SetEncChangeMsg)(m_lPort, m_hWnd, WM_ENC_CHANGE);
		NAME(VN_PLAY_SetEncTypeChangeCallBack(m_lPort, EncChange, (long)this));
		...
		else
		{
			...
			NAME(VN_PLAY_SetFileEndMsg)(m_lPort, m_hWnd, WM_FILE_END);

			NAME(VN_PLAY_SetFileRefCallBack)(m_lPort, FileRefDone, (DWORD)this);	

			if(!NAME(VN_PLAY_OpenFile)(m_lPort, fileName))
			{
				m_strPlayFileName = _T("");
				strError.Format("Open file failed(%s)", MyErrorToString(NAME(VN_PLAY_GetLastError)(m_lPort)));
				MessageBox(strError);
				throw 0;
			}

			m_dwMaxFileTime = NAME(VN_PLAY_GetFileTime)(m_lPort);
			if(!m_dwMaxFileTime)
			{
			...
			m_dwTotalFrames		= NAME(VN_PLAY_GetFileTotalFrames)(m_lPort);

			Play();
			SetWindowText((LPCTSTR) m_strPlayFileName);	
		}
		
		NAME(VN_PLAY_SetPicQuality)(m_lPort, m_bPicQuality);

		m_bOpen = TRUE;
		
		NAME(VN_PLAY_GetPictureSize)(m_lPort, &m_nWidth, &m_nHeight);
		NAME(VN_PLAY_SetVolume)(m_lPort,WORD(m_SoundSlider.GetPos()));
		m_pDisplayRegion->SetResolution(m_nHeight, m_nWidth);
	...
	//m_strPlayFileName = CString(fileName);
	//Open();
	//return;
...
void CPlayerDlg::OpenFile()
{
	...
	NAME(VN_PLAY_SetFileRefCallBack)(m_lPort, FileRefDone, (DWORD)this);
//	NAME(VN_PLAY_SetVerifyCallBack)(m_lPort, 0, 0xffffffff, VerifyFun, (DWORD) this); // verify the whole file;
//	NAME(VN_PLAY_SetDecCallBackMend)(m_lPort, DecCBFun, (long)this);	

	if(!NAME(VN_PLAY_OpenFile)(m_lPort, m_strPlayFileName.GetBuffer(m_strPlayFileName.GetLength())))
//	if(!NAME(VN_PLAY_OpenFile)(0, "J:\\we.dav"))
	{
		m_strPlayFileName = _T("");
		strError.Format("Open file failed(%s)", MyErrorToString(NAME(VN_PLAY_GetLastError)(m_lPort)));
		MessageBox(strError);
	...
	// try overlay mode!
//	NAME(VN_PLAY_SetOverlayMode)(m_lPort, TRUE, RGB(255,0,255));
	// try flip overlay mode
//	NAME(VN_PLAY_SetOverlayFlipMode)(m_lPort, TRUE);
	...
	if(m_bConvertAVI)
	{
		...
		NAME(VN_PLAY_GetPictureSize)(m_lPort, &dwWidth, &dwHeight);
...
//	NAME(VN_PLAY_SetDisplayBuf)(m_lPort,50);
//	NAME(VN_PLAY_SetDisplayBufAddtionalLen)(m_lPort, 6);

	Play();
//	VN_PLAY_SetPlayedTimeEx(m_lPort, 3000000);
// 	BOOL b = NAME(VN_PLAY_SetDisplayType)(m_lPort, 3);
//	VN_PLAY_SetCurrentFrameNum(m_lPort, 10000);
//	VN_PLAY_SetPlayPos(m_lPort, 1.2);
// 	DWORD dw = VN_PLAY_GetLastError(m_lPort);
	SetWindowText((LPCTSTR) m_strPlayFileName);
}
...
//	NAME(VN_PLAY_SetSourceBufCallBack)(m_lPort, 6000000, SourceBufFun, (DWORD)this, NULL);
	NAME(VN_PLAY_SetStreamOpenMode)(m_lPort, STREAME_FILE);
	m_dwHeadSize = NAME(VN_PLAY_GetFileHeadLength)();
	PBYTE pBuf = NULL;
	char csError[50];
...
#ifdef SPLIT_INPUT
	if(!NAME(VN_PLAY_OpenStreamEx)(m_lPort, (BYTE*)&stInfo, m_dwHeadSize, 6*1000*1024))
	{
		sprintf(csError, "Open stream failed(%s)", MyErrorToString(NAME(VN_PLAY_GetLastError)(m_lPort)));
		MessageBox(csError);
		delete []pBuf;
		pBuf = NULL;
		throw 0;
	}
	VN_PLAY_SyncToAudio(m_lPort, FALSE);
#else defined MIX_INPUT	

///	if(!NAME(VN_PLAY_OpenStream)(m_lPort, pBuf, sizeof(stInfo),  6*1000*1024))
///	{
	///	sprintf(csError, "Open stream failed(%s)", MyErrorToString(NAME(VN_PLAY_GetLastError)(m_lPort)));
	///	MessageBox(csError);
	///	delete []pBuf;
	...
	///	throw 0;
///	}
///	NAME(VN_PLAY_SyncToAudio)(m_lPort, FALSE);
#endif
...
{
	Stop();
	NAME(VN_PLAY_CloseStream)(m_lPort);
	if(m_hStreamFile)
	{
...
{
	DWORD nCurrentTime = NAME(VN_PLAY_GetPlayedTime)(m_lPort);

	TRACE("hytest: play position = %f!\n", NAME(VN_PLAY_GetPlayPos)(m_lPort));
	...
	{
#ifdef _FILE_POS
	float fPos = NAME(VN_PLAY_GetPlayPos)(m_lPort);
	...
	DWORD nCurrentFrame = NAME(VN_PLAY_GetCurrentFrameNum)(m_lPort);
...
//	INT nAdjust = (m_nPrePlayPos < nPos) ? 1 : 0;
	...
	// test
	// TRACE("Current frame rate:%d\n",NAME(VN_PLAY_GetCurrentFrameRate)(m_lPort));
	// TRACE("Current time use ms:%d\n",NAME(VN_PLAY_GetPlayedTimeEx)(m_lPort));
...
//		m_pMainMenu->EnableMenuItem(IDM_CUT_FILE, MF_GRAYED | MF_DISABLED);
		...
		if((HBITMAP)m_VDBmp != m_ctrlVideoPic.GetBitmap())
		{
			m_ctrlVideoPic.SetBitmap(m_VDBmp);
		}
...
void CPlayerDlg::TestCapability(DWORD nDeviceNum)
{
	CString csCap="";
#if (WINVER > 0x0400)
	int nFlag=NAME(VN_PLAY_GetCapsEx)(nDeviceNum);
#else
	int nFlag=NAME(VN_PLAY_GetCaps());
#endif
...
void CPlayerDlg::SetDisplayRegion(RECT Rect)
{
	...
	NAME(VN_PLAY_SetDisplayRegion)(m_lPort, 0, &Rect, GetDlgItem(IDC_SHOW)->m_hWnd, TRUE);
	UpdateWindow();
 	NAME(VN_PLAY_RefreshPlayEx)(m_lPort, 0);
}
/*************************************************************************/
/*************************************************************************/
/*************************************************************************/
...
///		NAME(VN_PLAY_SetCheckWatermarkCallBack)(m_lPort, funCheckWatermark, (DWORD)this);
		
		m_nSpeed = 0;
		ThrowB(IDM_THROW0);

		// set image sharpen level, default is 0
///		NAME(VN_PLAY_SetImageSharpen)(m_lPort, m_dwImageSharpenLevel);
		// set play mode, high fluid
///		NAME(VN_PLAY_SetPlayMode)(m_lPort, m_bHighFluid);
		...
		for(int i = IDM_DECODE_NORMAL; i <= IDM_DECODE_NONE; i ++ )
		{
			...
			{
///				NAME(VN_PLAY_SetDecodeFrameType)(m_lPort, i - IDM_DECODE_NORMAL);
				break;
			}
		}
		...
		if(m_bConvertAVI)
		{
///			NAME(VN_PLAY_SetDecCallBackMend)(m_lPort, DecCBFun, DWORD(this));
			m_bFileRefCreated = FALSE;
		}
		...
		if(m_bStreamType)
		{
			...
			NAME(VN_PLAY_ResetSourceBuffer)(m_lPort);
		...
		m_bSound = NAME(VN_PLAY_PlaySound)(m_lPort);	
	}	
...
void CPlayerDlg::Pause()
{
	if(m_enumState == State_Play)
	{
		NAME(VN_PLAY_Pause)(m_lPort, TRUE);
...
void CPlayerDlg::GotoStart() 
{
	...
	if(m_bFileRefCreated)
	{
		NAME(VN_PLAY_SetPlayedTimeEx)(m_lPort, 0);
	}
	else
	{
		NAME(VN_PLAY_SetPlayPos)(m_lPort, 0);
	}
}
...
//		int nCurFrame = NAME(VN_PLAY_GetCurrentFrameNum)(m_lPort);
		int count = 0;
		while( !NAME(VN_PLAY_SetPlayedTimeEx)(m_lPort, m_dwMaxFileTime * 1000 ) && count < 1)
		{
			m_dwMaxFileTime--;
			count++;
			//TRACE("FrameNum is :%d\n",nEndFrame);
...
//			{
//				NAME(VN_PLAY_SetCurrentFrameNum)(m_lPort, nCurFrame);
//				break;
//			}
		}
	}
	...
	{
		NAME(VN_PLAY_SetPlayPos)(m_lPort, 1);
	}
...
void CPlayerDlg::StepBackward() 
{
	...
	if(m_bFileRefCreated)
	{
		NAME(VN_PLAY_OneByOneBack)(m_lPort);
...
void CPlayerDlg::StepForward() 
{
	...
	// DWORD nCurrentFrame = NAME(VN_PLAY_GetCurrentFrameNum)(m_lPort);
	// NAME(VN_PLAY_SetCurrentFrameNum)(m_lPort,nCurrentFrame+1);
...
void CPlayerDlg::GetPic(char* pImage, DWORD nBufSize,long nWidth,long nHeight,long nType)
{
	CString sFilePath;
	CFile	clsFile;
	...
	if(m_nCapPicType == 1)
	{
///		if( !NAME(VN_PLAY_GetJPEG)(m_lPort, pImage, nBufSize, &pImageSize) )
	///	{
	///		return;
	///	}
	...
	{
///		if( !NAME(VN_PLAY_GetBMP)(m_lPort, pImage, nBufSize, &pImageSize) )
	///	{
	///		return;
	///	}
	...
	m_npic_bmp++;
		//BOOL bret = VN_PLAY_CatchPic(m_lPort,sFilePath.GetBuffer(0));
	}
...
void CPlayerDlg::Cappic() 
{
	// TODO: Add your control notification handler code here
	PBYTE	pImage		= NULL;
	DWORD   nBufSize	= m_nWidth * m_nHeight * 5;  //promise large enough(not less than one bmp or jpeg picture size) // 保证足够大即可(不会小于一幅bmp或者jpeg图像大小)
...
void CPlayerDlg::Sound() 
{
	...
	{
		if(NAME(VN_PLAY_PlaySound)(m_lPort))
		{
			m_ctrlBtnSound.SetIcon(IDI_SOUND_ENABLE);
			m_bSound = TRUE;
		}
...
#ifdef _WAVE_ADJ
		NAME(VN_PLAY_AdjustWaveAudio)(m_lPort, nSoundPos);
#else
		NAME(VN_PLAY_SetVolume)(m_lPort, WORD(nSoundPos));
#endif
...
///		VN_PLAY_GetPort(&m_lPort);
	...
	try
	{
		...
		NAME(VN_PLAY_SetPicQuality)(m_lPort, m_bPicQuality);
///		NAME(VN_PLAY_SetDeflash)(m_lPort, m_bDeflash);
///		NAME(VN_PLAY_SetPlayMode)(m_lPort, m_bHighFluid);
...
BOOL CPlayerDlg::SetDevice(UINT nID)
{
	...
	if(nVal >= 1)
	{
		...
		if(NAME(VN_PLAY_SetDDrawDevice)(m_lPort, nDeviceSeq + 1))
		{
			bFunctionOK = TRUE;
		}
		
		NAME(VN_PLAY_SetDDrawDeviceEx)(m_lPort, 1, nDeviceSeq + 1);
		m_pMainMenu->CheckMenuItem(nID, MF_CHECKED);
	}
#endif
...
{
	NAME(VN_PLAY_ThrowBFrameNum)(m_lPort, nID - IDM_THROW0);
...
{
///	NAME(VN_PLAY_SetDecodeFrameType)(m_lPort, nID - IDM_DECODE_NORMAL);
...
void CPlayerDlg::DisplayType(UINT nID)
{
	...
	{
		NAME(VN_PLAY_SetDisplayType)(m_lPort, DISPLAY_NORMAL);
	}
	else
	{
		NAME(VN_PLAY_SetDisplayType)(m_lPort, DISPLAY_QUARTER);
	}
}
...
void CPlayerDlg::SelectTimer(UINT nID)
{
	...
	if(nID == IDM_TIMER1)
	{
		NAME(VN_PLAY_SetTimerType)(m_lPort, TIMER_1, 0);
	}
	else
	{
		NAME(VN_PLAY_SetTimerType)(m_lPort, TIMER_2, 0);
	}
}
...
void CPlayerDlg::ResetBuf() 
{
	...
	NAME(VN_PLAY_ResetBuffer)(m_lPort, BUF_VIDEO_RENDER);
	NAME(VN_PLAY_ResetBuffer)(m_lPort, BUF_AUDIO_RENDER);
	NAME(VN_PLAY_ResetBuffer)(m_lPort, BUF_VIDEO_SRC);
	NAME(VN_PLAY_ResetBuffer)(m_lPort, BUF_AUDIO_SRC);
}
...
void CPlayerDlg::CappicPath() 
{
	...
	bi.lpszTitle	  = "select folder please";//"请选择目录";
...
char* CPlayerDlg::MyErrorToString(DWORD error)
{
    ...
    case 7:
      return "File open failed when calling API VN_PLAY_OpenFile.\0";
	case 8:
	...
	case 11:
      return "The input source buffer has overflowed when calling API VN_PLAY_InputData.\0";
	case 12:

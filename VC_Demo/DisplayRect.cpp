...
void CDisplayRect::InitShow()
{
    ...
    NAME(VN_PLAY_SetDisplayRegion)(m_lPort, 1, &m_rcRect, GetDlgItem(IDC_DISWND)->m_hWnd, TRUE);
	NAME(VN_PLAY_RefreshPlayEx)(m_lPort, 1);
}
...
BOOL CDisplayRect::SetDevice(UINT nSeq)
{
	...
	if(nVal >= 1)
	{
		if(NAME(VN_PLAY_SetDDrawDeviceEx)(m_lPort, 1, nSeq + 1))
		{
			bFunctionOK = TRUE;
		}
	}
...
void CDisplayRect::OnDestroy() 
{
	...
	// TODO: Add your message handler code here
	NAME(VN_PLAY_SetDisplayRegion)(m_lPort, 1, &m_rcRect, NULL, FALSE);
	m_bValid = FALSE;	

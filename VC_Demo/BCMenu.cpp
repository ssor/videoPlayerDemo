...
void BCMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	...
	else
	{
		...
		if(lpDIS->itemData != NULL)
		{
			...
			if(xoffset==0&&xp_draw_3D_bitmaps&&bitmap&&bitmap->GetImageCount()>2)
			{
				CanDraw3D=TRUE;
			}
...
BOOL BCMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,CBitmap *bmp)
{
	if(bmp)
	{
		...
		{
			temp.Add(bmp,m_bitmapBackground);
		}
		else 
		{
...
void BCMenu::SynchronizeMenu(void)
{
	...
	for(j=0;j<GetMenuItemCount();++j)
	{
		...
		if(state&MF_POPUP)
		{
			...
			if(!mdata)
			{
				mdata=NewODMenu(j, (state&0xFF)|MF_BYPOSITION|MF_POPUP|MF_OWNERDRAW,
					            submenu,string);
			}
		...
		else if(state&MF_SEPARATOR)
		{
			...
			if(!mdata)
			{
				mdata=NewODMenu(j, state|MF_BYPOSITION|MF_SEPARATOR|MF_OWNERDRAW,
					            0,_T(""));//SK: modified for Unicode correctness
			}
			else 
			{
						...
						COLORREF bgcolor)
{
	...
	if (hDC)
	{
		...
		if (bwDC)
		{
			...
			VOID *pbitsBW;
			HBITMAP hbmBW = CreateDIBSection(bwDC, (LPBITMAPINFO)&RGBBWBITMAPINFO, 
...
BOOL BCMenu::AddBitmapToImageList(CImageList *bmplist,UINT nResourceID)
{
	...
	else
	{
		...
		if(hbmp)
		{
			...
			if(m_bitmapBackgroundFlag)
			{
				if(bmplist->Add(&bmp,m_bitmapBackground)>=0)
				{
					bReturn=TRUE;
				}
			}
		...
		else
		{
			...
			if(mybmp.LoadBitmap(nResourceID))
			{
				if(m_bitmapBackgroundFlag)
				{
					if(bmplist->Add(&mybmp,m_bitmapBackground)>=0)
					{
						bReturn=TRUE;
					}
				}
...
BOOL BCMenu::DrawXPCheckmark(CDC *dc, const CRect& rc, HBITMAP hbmCheck,COLORREF &colorout)
{
	...
	if (!hbmCheck)
	{
		DrawCheckMark(dc,rc.left+5,rc.top+4,GetSysColor(COLOR_MENUTEXT));
	}
	else 
	{
		DrawRadioDot(dc,rc.left+5,rc.top+4,GetSysColor(COLOR_MENUTEXT));
...
void BCMenu::ColorBitmap(CDC* pDC,CBitmap& bmp,CSize bitmap_size,CSize icon_size,COLORREF fill,COLORREF border,int hatchstyle)
{
	...
	{
		m_newclrBack=GetSysColor(COLOR_MENU);
	}
